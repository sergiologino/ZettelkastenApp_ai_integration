...
- **2026-04-26**: `POST /api/social/posts` расширен для Telegram-вложений: `attachments[]` принимает документы, изображения и видео через `base64` или `url`, поддержаны одиночные файлы и несколько вложений через `sendMediaGroup`. В логи пишутся только метаданные вложений, без содержимого файлов.
