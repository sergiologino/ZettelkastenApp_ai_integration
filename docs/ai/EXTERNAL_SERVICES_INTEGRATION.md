...
- Если `images[]` передан и из него удалось извлечь изображения, он считается полным списком входов; top-level поля `image1Base64`, `sourceImageBase64` и т.п. дополнительно не подмешиваются.
- Для Grok Imagine edit действует лимит провайдера: максимум 3 входных изображения.
