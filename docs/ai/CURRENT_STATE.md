...
- **Virtual try-on**: `VirtualTryOnClient` использует Grok Imagine или явно настроенный backend **FASHN** (`FashnClient`: person+garment photo `tryon-max`, video `tryon-max→image-to-video`) / **Kling** (`KlingVirtualTryOnClient`: person+garment photo Kolors, video `try-on→image2video`). Pollinations удалён: нет клиента, нет выбора провайдера, нет fallback-маршрута; V027 удаляет старые записи из БД. Сети: `fashn-tryon-max`, `fashn-tryon-video`, `kling-kolors-tryon`, `kling-tryon-video` (V023). `fashn-product-to-model` — только flat-lay→новая модель, не ваш человек.
