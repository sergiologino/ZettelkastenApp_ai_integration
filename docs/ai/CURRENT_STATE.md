...
- **AI**: `AiController`, `AiOrchestrationService`, `NeuralClientFactory`; клиенты OpenAI, Claude, Mistral, GigaChat, Whisper, Qwen, DeepSeek, YandexGpt, FASHN, Kling, SeasonHitVideo; поддержка текстовых, транскрипции, image/video сетей (миграции V014, V015+). Провайдеры **google** и **xai** маршрутизируются через `OpenAiClient` (OpenAI-совместимый API).
...
- **Virtual try-on и generic image edit**: `VirtualTryOnClient` использует Grok Imagine или явно настроенный backend **FASHN** (`FashnClient`: person+garment photo `tryon-max`, video `tryon-max→image-to-video`) / **Kling** (`KlingVirtualTryOnClient`: person+garment photo Kolors, video `try-on→image2video`). Для одежды `personImageBase64 + garmentImageBase64` остаётся обязательным. Для не-одежных image-edit сценариев на Grok-backed `wibestyle-vton` payload может передать `images[]`; если `garmentImageBase64` отсутствует, сервис отправляет изображения в порядке клиента без доменной проверки person+garment. Pollinations удалён: нет клиента, нет выбора провайдера, нет fallback-маршрута; V027 удаляет старые записи из БД. Сети: `fashn-tryon-max`, `fashn-tryon-video`, `kling-kolors-tryon`, `kling-tryon-video` (V023). `fashn-product-to-model` — только flat-lay→новая модель, не ваш человек.
- **Kling API keys**: для провайдера `kling` поле `apiKey` в админке хранит Kling Access Key, поле `apiSecret` хранит Kling Secret Key (`neural_networks.api_secret_encrypted`). Секреты не возвращаются открытым текстом: `NetworkDTO.hasApiKey`/`hasApiSecret` подтверждают их наличие, а пустые значения при редактировании сохраняют прежние ключи. Перед вызовом Kling сервис генерирует HS256 JWT (`iss=Access Key`, подпись `Secret Key`) и отправляет его как `Authorization: Bearer <jwt>`. Внешний контракт `/api/ai/process` и `X-API-Key` для клиентских приложений не меняется.
- **Kling international API (2026-07-02)**: submit/poll запросы используют `https://api-singapore.klingai.com/v1/...`; миграция V025 заменяет устаревший `https://api.klingai.com`, а клиент нормализует старый URL на случай ручной конфигурации.
