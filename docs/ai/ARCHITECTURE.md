...
| **Client** | Внешние AI: NeuralClientFactory, OpenAiClient, PollinationsClient (BaseNeuralClient) |
| **Security** | JwtAuthFilter, ApiKeyAuthFilter, SecurityConfig, EncryptionService |

## Потоки

1. **AI-запрос**: приложение → API (JWT или API Key) → AiController → AiOrchestrationService → NeuralClientFactory → провайдер (OpenAI/Pollinations и т.д.).
...
- AI-провайдеры (OpenAI API, Pollinations и др.).
