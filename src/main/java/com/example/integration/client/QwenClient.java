    ...
    protected HttpHeaders prepareHeaders(NeuralNetwork network) {
        ...
        if (network.getApiKeyEncrypted() != null && !network.getApiKeyEncrypted().isEmpty()) {
            try {
                ...
                System.err.println("❌ [QwenClient] Ошибка расшифровки API ключа в prepareHeaders: " + e.getMessage());
                throw new RuntimeException("Ошибка расшифровки API ключа для Qwen", e);
            }
        }
